...
echo "=================================="
echo "IPv6 WireGuard Manager 原生安装"
echo "=================================="
echo ""
...
ExecStart=$APP_HOME/backend/venv/bin/uvicorn app.main:app --host 127.0.0.1 --port 8000
Restart=always
RestartSec=10
...
show_result() {
    ...
    echo ""
    echo "🛠️  管理命令："
    echo "   查看后端状态: sudo systemctl status ipv6-wireguard-manager"
    echo "   查看后端日志: sudo journalctl -u ipv6-wireguard-manager -f"
    echo "   重启后端: sudo systemctl restart ipv6-wireguard-manager"
    ...
    echo "⚠️  安全提醒："
    echo "   请在生产环境中修改默认密码"
    echo "   配置文件位置: $APP_HOME/backend/.env"
    echo ""
}
