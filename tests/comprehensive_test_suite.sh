...
declare -g WINDOWS_COMPATIBILITY_TESTS=0
...
# 运行 Windows 兼容性测试
run_windows_compatibility_tests() {
    echo -e "${BLUE}=== 运行 Windows 兼容性测试 ===${NC}"
    ...
    case "$env_type" in
        "wsl")
            ((total_tests++))
            if test_wsl_compatibility; then
                ((passed_tests++))
            fi
            ;;
        "msys2")
            ((total_tests++))
            if test_msys2_compatibility; then
                ((passed_tests++))
            fi
            ;;
        "powershell")
            ((total_tests++))
            if test_powershell_compatibility; then
                ((passed_tests++))
            fi
            ;;
        "git_bash")
            ((total_tests++))
            if test_git_bash_compatibility; then
                ((passed_tests++))
            fi
            ;;
        *)
    ...
    # 通用 Windows 兼容性测试
    ((total_tests++))
    if test_windows_path_handling; then
        ((passed_tests++))
    fi
    
    ((total_tests++))
    if test_windows_network; then
        ((passed_tests++))
    fi
