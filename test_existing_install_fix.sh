...
# 创建测试目录
TEST_DIR="/tmp/existing-install-test-$(date +%s)"
mkdir -p "$TEST_DIR"
cd "$TEST_DIR" || exit
...
echo "创建空目录: $INSTALL_DIR, $CONFIG_DIR, $LOG_DIR"
...
echo "创建文件: $INSTALL_DIR/test.txt"
...
echo "test" > "$BIN_DIR/ipv6-wireguard-manager"
echo "创建关键文件: $BIN_DIR/ipv6-wireguard-manager"
...
# 清理
cd / || exit
rm -rf "$TEST_DIR" "$INSTALL_DIR" "$CONFIG_DIR" "$LOG_DIR" "$BIN_DIR" "$SERVICE_DIR"
echo "✓ 测试完成，临时目录已清理"
